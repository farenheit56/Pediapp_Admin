<template>
  <q-card class="table-bg no-shadow">
    ...
    <q-card-section class="q-pa-none">
      <q-table class="table-bg" :data="products" :columns="columns" :filter="filter" :pagination="pagination">
        ...
        <template v-slot:top-right>
          <q-btn color= positive class="float-right text-capitalize" icon="add" @click="addProductDialog = true"/>
        </template>
        <template v-slot:body-cell-Action="props">
          <q-td :props="props">
            <q-btn icon="edit" size="sm" flat dense @click="editProduct(props.row)"/>
            <q-btn icon="delete" size="sm" class="q-ml-sm" flat dense @click="deleteProduct(props.row)"/>
          </q-td>
        </template>
        <template v-slot:body-cell-Image="props">
          <q-td :props="props">
            <q-img :src="`http://localhost:3000/images/` + props.row.image_url" height="auto" width="100px" :ratio="1"/>
          </q-td>
        </template>
      </q-table>
    </q-card-section>
      ...
      <q-card style="min-width: 750px">
        ...
        <q-card-section class="q-pt-none">
          <div class="row">
            ...
            <div class="col q-mx-sm"><q-input dense v-model="editedItem.price" label="Precio"/></div>
          </div>
          ...
          <div class="row q-mt-md justify-center">
            <div class="col-6 q-mx-sm">
              ...
              @rejected="onRejected">
                ...
                <template v-slot:append>
                  <q-icon v-if="editedItem.image_url !== null" name="close" @click.stop="editedItem.image_url = null" class="cursor-pointer" />
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {name: 'Image', label: 'Imagen', field: 'image_url', sortable: true, align: 'left'},
